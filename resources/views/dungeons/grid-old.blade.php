<x-app-layout>
    <div class="p-6 space-y-4" x-data="{ scaleDungeon: 1 }">
        ...
        <div class="flex gap-2 items-center justify-center mb-2">
            <a href="#" @click.prevent="scaleDungeon = Math.max(0.5, scaleDungeon - 0.5)" class="bg-gray-700 text-white px-4 py-2 rounded hover:bg-gray-600">Zoom Out</a>
            <a href="#" @click.prevent="scaleDungeon = Math.min(6, scaleDungeon + 0.5)" class="bg-gray-700 text-white px-4 py-2 rounded hover:bg-gray-600">Zoom In</a>
        ...
        <div class="flex justify-center overflow-auto border border-gray-500 rounded-lg bg-black p-2">
            <div class="grid gap-0" x-data="dungeonExplorer('{{ $startX }}', '{{ $startY }}')" x-bind:style="'scale: ' + scaleDungeon + '; ' + 'grid-template-columns: repeat({{ count($grid[0]) }}, 1fr); width: fit-content;'">
                @foreach ($grid as $y => $row)
                    @foreach ($row as $x => $cell)
                        ...
                        >
                            ...
                            {{-- Masking Layer --}}
{{--                            <div class="absolute inset-0 bg-black" x-show="!explored.includes('{{ $key }}')" style="opacity: 1;"></div>--}}
                        </div>
                    @endforeach
                @endforeach
            </div>
